...
static void cpuid(uint32_t leaf, uint32_t subleaf, uint32_t *a, uint32_t *b, uint32_t *c, uint32_t *d) {
    ...
    __asm__ volatile(
        ...
        "movl %%ebx, %1\n\t"
        "popl %%ebx\n\t"
        : "+a"(eax_inout), "=m"(ebx_out), "+c"(ecx_inout), "=d"(edx_out)
        :
        : "cc"
    );
    *a = eax_inout;
...
void command_info(void) {
    ...
    if (max_basic >= 1) {
        ...
        if (d & (1u << 0)) print_feature("FPU");
        if (d & (1u << 9)) print_feature("APIC");
        if (d & (1u << 23)) print_feature("MMX");
        if (d & (1u << 25)) print_feature("SSE");
        if (d & (1u << 26)) print_feature("SSE2");

        if (c & (1u << 0)) print_feature("SSE3");
        if (c & (1u << 9)) print_feature("SSSE3");
        if (c & (1u << 19)) print_feature("SSE4.1");
        if (c & (1u << 20)) print_feature("SSE4.2");
        if (c & (1u << 28)) print_feature("AVX");
