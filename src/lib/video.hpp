...
void redraw_line(int line_start, const char *buffer, int length,
                 int cursor_pos);
int get_cursor_pos();
...
inline void video_print_string(const char *str) {
  lib::video::print_string(str);
}
inline void video_redraw_line(int line_start, const char *buffer, int length,
                              int cursor_pos) {
  lib::video::redraw_line(line_start, buffer, length, cursor_pos);
}
...
inline void video_set_color(unsigned char fg, unsigned char bg) {
  lib::video::set_color(fg, bg);
}
inline void video_set_color_attr(unsigned char attr) {
  lib::video::set_color_attr(attr);
}
inline int video_get_screen_width() { return lib::video::get_screen_width(); }
inline int video_get_screen_height() { return lib::video::get_screen_height(); }
}
